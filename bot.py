...
# Указываем путь внутри подключённого в Railway постоянного тома (Volume) —
# так настройки не теряются при каждом новом деплое.
# Если том не подключён (например, при локальном запуске), используем
# обычный путь рядом со скриптом.
REMINDERS_FILE = "/data/reminders.json" if os.path.isdir("/data") else "reminders.json"
